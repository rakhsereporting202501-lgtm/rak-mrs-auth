...
export default function AppLauncher() {
  ...
  const name = getDisplayName(role, user) || user?.email || 'User';
      ...
      <div className="max-w-5xl mx-auto px-4 py-5">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            ...
            <div>
              <div className="text-base font-semibold text-gray-900">RAK Applications</div>
        ...
        <div className="pt-16 sm:pt-24">
          <div className="text-center mb-8">
            <div className="text-2xl sm:text-3xl font-bold text-gray-900">Select Application</div>
            <div className="mt-2 text-sm text-gray-600">Choose where you want to work.</div>
          </div>
          <div className="grid sm:grid-cols-2 gap-4">
            <AppCard
              title="RAK IMS"
              description="Requests, inventory, reporting, users, and profile tools."
              Icon={Boxes}
              onClick={() => nav('/requests')}
            />
            <AppCard
              title="RAK WP"
              description="Daily work plans for assigning employees and equipment teams to projects."
              Icon={ClipboardList}
              onClick={() => nav('/wp')}
            />
          </div>
          <div className="mt-6 flex items-center justify-center text-xs text-gray-500 gap-2">
            ...
            <span>One login, separate application areas.</span>
          </div>
        </div>
      </div>
    </div>
